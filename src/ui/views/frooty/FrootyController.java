package ui.views.frooty;

import core.abc.*;
import core.excel.*;
import core.excel.converter.*;
...
public class FrootyController implements Initializable
{
    ...
    public void initialize(URL url, ResourceBundle resourceBundle)
    {
        ...
        initialFolderPathGoogle = prefs.get(UserPreferences.FROOTY_GOOGLE_LAST_OPEN_FOLDER_PATH, null);
        initialFolderPathDysrup = prefs.get(UserPreferences.FROOTY_DYSRUP_LAST_OPEN_FOLDER_PATH, null);
        initialFolderPathReport = prefs.get(UserPreferences.FROOTY_REPORT_LAST_OPEN_FOLDER_PATH, null);
    ...
    private void fileIconPressed(MouseEvent event)
    {
        ...
        } else if (dysrupOriginalIcon.equals(imgView)) {
            chosenFile = FileBrowser.openFromSpecificFolder(
        ...
        if (chosenFile != null)
        {
            ...
            if (imgView == googleOriginalIcon) {
                ...
                prefs.put(UserPreferences.FROOTY_GOOGLE_LAST_OPEN_FOLDER_PATH, initialFolderPathGoogle);
                ...
                prefs.put(UserPreferences.FROOTY_DYSRUP_LAST_OPEN_FOLDER_PATH, initialFolderPathDysrup);
                ...
                prefs.put(UserPreferences.FROOTY_REPORT_LAST_OPEN_FOLDER_PATH, initialFolderPathReport);
    ...
    private void readFiles()
    {
        ...
        if (!googleFilePath.isEmpty()) {
            try {
                googleSheetData = readSheetData(googleFilePath, MandatoryFieldsFrootyGoogle.MANDATORY_FIELDS_GOOGLE);
            } catch (Exception e) {
    ...
    private ArrayList<ProductRequest> getProductRequestsDysrup() throws Exception
    {
        ...
        dysrupSheetData = Filter.filterEquals(MandatoryFieldsDysrup.CLIENTE, "Congebras", dysrupSheetData);

        if (dysrupSheetData.size() == 0)
        {
            errorMessage = "Não foi possível filtrar as linhas do arquivo pela coluna " + "\"" + MandatoryFieldsDysrup.CLIENTE + "\"\n\n";
            errorMessage = errorMessage + "Verifique se o nome da coluna está correto.\nSe o nome da coluna estiver correto, verifique se a marca \"Congebras\" possui esse mesmo nome (os acentos podem ser desconsiderados) na coluna " + "\"" + MandatoryFieldsDysrup.CLIENTE + "\"";
            throw new Exception(errorMessage);
        }
